...
min_pct = float(df["Gain_Loss_Pct"].min())
max_pct = float(df["Gain_Loss_Pct"].max())
selected_pct_range = st.sidebar.slider(
    "Gain/Loss %",
    min_value=min_pct,
    max_value=max_pct,
...
df = df[
    ...
    & df["Gain_Loss_Pct"].between(*selected_pct_range)
]
...
overview_tab, holdings_tab, charts_tab = st.tabs(["Overview", "Holdings", "Charts"])
...
with charts_tab:
    if df.empty:
        ...
        with chart_col2:
            ...
            bar_colors = [
                "#2ecc71" if pct >= 0 else "#e74c3c"
                for pct in df_sorted["Gain_Loss_Pct"]
            ]
            fig_bar = px.bar(
                df_sorted,
                x="Ticker",
